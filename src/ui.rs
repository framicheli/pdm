...
pub fn ui(frame: &mut Frame, app: &mut App) {
    ...
    if let Some(msg) = &app.notification {
        ...
        let footer_text = match app.current_screen {
            ...
            CurrentScreen::Editing => "Esc: Back, Tab: Next Section, Enter: Edit, Space: Toggle, Ctrl+S: Save",
...
fn render_file_explorer(frame: &mut Frame, app: &mut App, area: Rect) {
    let items: Vec<ListItem> = app.file_explorer.files
        .iter()
        .map(|path| {
    ...
    let list = List::new(items)
        .block(Block::default().borders(Borders::ALL).title(format!("File Explorer: {}", app.file_explorer.current_dir.display())))
...
fn render_editing_screen(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    if app.sections.is_empty() {
        let msg = Paragraph::new("No configuration loaded or empty.")
            .alignment(Alignment::Center);
    ...
    // List of items
    let items: Vec<ListItem> = current_section.items.iter().map(|entry| {
